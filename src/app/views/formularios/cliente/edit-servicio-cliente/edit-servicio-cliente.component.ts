...
@Component({
  selector: 'app-edit-servicio-cliente',
  templateUrl: './edit-servicio-cliente.component.html',
  styleUrls: ['./edit-servicio-cliente.component.css']
})
export class EditServicioClienteComponent implements OnInit {
  ...
  constructor(
    ...
    private _formBuilder: FormBuilder, public usuariosService: UsuariosService, private normasServices: NormasService, private _formsService: FormulariosService, public storage: AngularFireStorage, private dialogRef: MatDialogRef<EditServicioClienteComponent>, private _service:ServiciosService
  ) { }
      ...
      this._formsService.updateForm(this.servicio).subscribe(
        resu => {
          ...
          for(let i=0; i<5; i++){
            ...
            else if(this.servicio.anexos.includes(i.toString()) == true && this.array_files_original.includes(i.toString()) == true){
              var ref = storageRef.child(`Pre_Servicio/${this.servicio.Id_Form}/${this.arrayAnexos[i]}.rar`);
              ref.delete().then(res => {
                const jszip = new JSZip();
                ...
                if(this.arrayCheckbox[i].file.length > 0){
                  ...
                  jszip.generateAsync({ type: 'blob' }).then(function(content) {
                    var referencia = storageRef.child(`Pre_Servicio/${id}/${name}.rar`);
                    referencia.put(content).then(() => {
                      console.log(`Se resubio el ${name}`);
                    })
                  });
                }
              })
          ...
          this._formsService.updateForm(this.servicio).subscribe(
            resu => {
              ...
              for(let i=0; i<2; i++){
                ...
                else if(this.servicio.anexos_dictamen.includes(i.toString()) == true && this.array_files_d_original.includes(i.toString()) == true){
                  var ref = storageRef.child(`Pre_Servicio/${this.servicio.Id_Form}/${this.arrayAnexosDictamen[i]}.rar`);
                  ref.delete().then(res => {
                    const jszip = new JSZip();
