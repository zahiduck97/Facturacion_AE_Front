...
@Component({
  selector: 'app-nuevo-servicio-cliente',
  templateUrl: './nuevo-servicio-cliente.component.html',
  styleUrls: ['./nuevo-servicio-cliente.component.css']
})
export class NuevoServicioCLienteComponent implements OnInit {
  ...
  // Constructor
  constructor(private _formBuilder: FormBuilder, public usuariosService: UsuariosService, private normasServices: NormasService, private _formsService: FormulariosService, public storage: AngularFireStorage, private dialogRef: MatDialogRef<NuevoServicioCLienteComponent>, private _service:ServiciosService, public _login:LoginService) {}
  ...
  async ngOnInit() {
    ...
    this.dictamenFormGroup = this._formBuilder.group({
      pedimento: ['', Validators.compose([Validators.required, Validators.pattern('^[0-9]')])],
  ...
  enviarForm(){
    this.preloaderActivo = true;
    this.desactivado = true;
    this.servicio.Id_Usuario = this.UsuarioId;
    ...
    console.log(this.servicio, this.UsuarioId);
